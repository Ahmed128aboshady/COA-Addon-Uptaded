...
try:
    ...
    def prepare_domain_v2(dom_tuple):
        return [dom_tuple]
    def compute_domain(dom_tuple, model_name):
        pass
...
class BaseModel(models.AbstractModel):
    ...
    def _get_view(self, view_id=None, view_type='form', **options):
        ...
        if readonly_access_id:
            if view_type in ['form', 'list', 'kanban', 'gantt','pivot','graph']:
                arch.attrib.update({'create': 'false', 'delete': 'false'})
        # if readonly_access_id:
        #     if view_type == 'form':
        #         arch.attrib.update({'create': 'false', 'delete': 'false'})
        ...
        #         arch.attrib.update({'create': 'false', 'delete': 'false'})
            ...
            if access_model_recs:
                ...
                if view_type in ['form', 'list', 'kanban', 'gantt','pivot','graph']:
                            arch.attrib.update({'create': create, 'delete': delete})
    ...
    def _get_access_management_domain_record(self, model=False):
        ...
        try:
            if model:
                # Guard: only run when the module is fully installed (prevents transaction corruption on fresh install)
                self.env.cr.execute(
                    "SELECT state FROM ir_module_module WHERE name='simplify_access_management_v19' LIMIT 1"
                )
