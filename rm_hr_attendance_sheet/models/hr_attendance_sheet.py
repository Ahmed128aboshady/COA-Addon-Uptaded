            ...
            if rec.employee_id and rec.check_in:
                ...
                for leave_id in leave_ids:
                    if leave_id.holiday_status_id.work_entry_type_id.id == self.env.ref(
                            'hr_work_entry.work_entry_type_unpaid_leave').id:
                    ...
                    if leave_id.holiday_status_id.work_entry_type_id.id == self.env.ref(
                            'hr_work_entry.work_entry_type_legal_leave').id:
                        if leave_id.request_date_from <= rec.check_in.date() <= leave_id.request_date_to:
                            raise ValidationError(_('There Is No Attendance For Employee %s' % rec.employee_id.name))
...
class hrPayslip(models.Model):
    ...
    def _get_total_absence(self):
        for rec in self:
            ...
            rec.total_absence = 0
            if rec.absence_num:
                amount_day = (rec.employee_id.wage + rec.employee_id.trans_allowance_val) / days
    ...
    def _get_workday_lines(self):
        for rec in self:
            if not rec.is_bool:
                ...
                total_unpaid_leave_hours = rec.attendance_sheet_id.unpaid_leave * rec.employee_id.resource_calendar_id.hours_per_day
                total_paid_leave_hours = rec.attendance_sheet_id.paid_leave * rec.employee_id.resource_calendar_id.hours_per_day
                total_num_att_hours = rec.attendance_sheet_id.num_att * rec.employee_id.resource_calendar_id.hours_per_day
...
class AttendanceSheet(models.Model):
    ...
    def _compute_total_unpaid_leave(self):
        for rec in self:
            ...
            if rec.unpaid_leave:
                amount_day = rec.employee_id.total_package_val / days
    ...
    def onchange_employee(self):
        ...
        if not self.employee_id.att_policy_id:
            raise ValidationError(_(
                "Employee %s does not have attendance policy" % employee.name))
        self.att_policy_id = self.employee_id.att_policy_id
    ...
    def get_attendances(self):
        for att_sheet in self:
            ...
            for day in all_dates:
                ...
                work_intervals = calendar_id.att_get_work_intervals_new(emp, day_start,
                                                                        day_end, tz)
            ...
            leave_ids = self.env['hr.leave'].search(
                [('employee_id', '=', att_sheet.employee_id.id), ('request_date_from', '>=', att_sheet.date_from),
            ...
            for leave in leave_ids:
                ...
                if leave.holiday_status_id.work_entry_type_id.id == self.env.ref(
                        'hr_work_entry.work_entry_type_legal_leave').id:
                    paid_leave += leave.number_of_days
    ...
    def action_create_payslip(self):
        ...
        for sheet in self:
            ...
            new_payslip = payslip_obj.new({
                ...
                'struct_id': sheet.employee_id.structure_id.id,
