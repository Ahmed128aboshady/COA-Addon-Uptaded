...
class StockLocation(models.Model):
    ...
    def _search(self, domain, offset=0, limit=None, order=None, *, active_test=True, bypass_access=False):
        if not self.env.su and active_test and self._is_location_restriction_enabled():
            allowed_location_ids = self._get_user_allowed_location_ids()
            if allowed_location_ids:
                all_allowed = self.sudo().search([
                    '|', ('usage', 'in', ['supplier', 'customer', 'inventory', 'production']),
                    '|', ('id', 'child_of', allowed_location_ids),
                         ('id', 'parent_of', allowed_location_ids),
                ])
                domain = [('id', 'in', all_allowed.ids)] + list(domain or [])
