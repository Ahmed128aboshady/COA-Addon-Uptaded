...
class BomConsumptionWizard(models.TransientModel):
    ...
    def _get_productions(self):
        ...
        # Odoo 19: date field on mrp.production is 'date_start'
    ...
    def _get_actual_qty(self, move):
        ...
        def _walk(mv, visited):
            ...
            if mv.move_orig_ids:
                ...
                # Leaf move – count quantity here (Odoo 19: qty_done → quantity)
                if picking_code == 'mrp_operation':
                    # 1-step: the MO move itself IS the consumption
                    return sum(
                        ml.quantity for ml in mv.move_line_ids
                        if ml.state == 'done'
                    )
                else:
                    # Pick / internal / outgoing move – real stock withdrawal
                    return sum(
                        ml.quantity for ml in mv.move_line_ids
                        if ml.state == 'done'
                    )
