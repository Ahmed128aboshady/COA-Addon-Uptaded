...
REPORT_STATEMENT = 'coa_direct_print_m.action_report_partner_statement'
REPORT_PAYMENT   = 'coa_direct_print_m.action_report_payment_receipt'
...
class CoaDirectPrint(http.Controller):
    ...
    # Routes
    # ------------------------------------------------------------------
    @http.route('/coa_direct_print_m/print/<string:model>/<int:res_id>/<string:report_key>',
                type='http', auth='user')
    def print_page(self, model, res_id, report_key, **kwargs):
        ...
        html = PRINT_PAGE % {
            ...
            'pdf_url': '/coa_direct_print_m/pdf/%s/%s/%s' % (model, res_id, report_key),
        }
        return request.make_response(
            html, headers=[('Content-Type', 'text/html; charset=utf-8')])

    @http.route('/coa_direct_print_m/pdf/<string:model>/<int:res_id>/<string:report_key>',
                type='http', auth='user')
